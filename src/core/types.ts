...
/**
 * One tool call of an assistant message. `text` and `isError` mirror the
 * outcome for hosts that keep it on the call itself rather than apart.
 */
...
/**
 * One transcript message, in the neutral shape the scoring works on. Pi's own
 * messages are mapped onto it by `src/pi/messages.ts`.
 */
...
export interface CompactOptions {
  ...
  /**
   * Extra restriction on which calls may be dropped at all. A call the
   * predicate rejects is pinned, so it is neither asked about nor touched.
   */
...
export interface CompactResult {
  ...
  /** Every paired call considered, so a host can apply the decisions to its own messages. */
  calls: ToolCall[];
  stats: {
